...
TH2D* h2d = new TH2D("h2d", ";Momentum of the Incoming Neutrino(Gev/c);Momentum of the Incoming Neutrino Reconstruceted(Gev/c)", 300, 0 , 300, 300, 0 , 300);
...
for(int ientry = 0; i >= ientry ; ++ientry){
    ...
    */
    ...
    //determina o angulo do meu leptao 

    teta = atan(pyl/pxl); 
    phi = acos(pzl/modl);

    pmx= ptotx/(sin(phi)*cos(teta));
    pmy= ptoty/(sin(phi)*sin(teta));
    
    c=sqrt(ptotx*ptotx+ptoty*ptoty)/cos(phi);
